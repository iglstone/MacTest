...
static int _check_result(const char * s, const char * line)
{
...
static int _get_sensor_info(hokuyo_t * h, const char * caller, int mode)
{
	...
	{
		return error_return(caller2, "response does not match1:");
	}
	...
	{
		return error_return(caller2, "response does not match2:");
	}
...
int hokuyo_get_scan(void * v, const char * caller, int * range)
{
	hokuyo_t * h = (hokuyo_t *)v;
	...
	{
		/* new scan is ready! */
		memcpy(range, h->data, h->num_data*sizeof(int));
		n_range = h->num_data;
		_set_scan_unready(h);	    
	} 
	else 
